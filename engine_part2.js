...
const TimelineController = {
    ...
    startTimeline: function() {
        ...
        // Immediate feedback so user knows something happened
        ...
        document.getElementById('horizon-glow').style.opacity = '0.3';
        document.getElementById('volumetric-light').style.opacity = '0.5';
...
function bootApp() {
    console.log('[IRON GRIND] bootApp running');
    ...
    if (!startBtn || !skipBtn || !loadingText || !startContainer) {
        console.error('[IRON GRIND] missing UI elements', { startBtn, skipBtn, loadingText, startContainer });
        return;
    }

    let started = false;

    // Button is ALWAYS clickable — never block on boot
    ...
    // Optional progress text (does not gate the button)
    let bootProgress = 0;
    ...
    function beginCinematic(e) {
        if (started) return;
        started = true;
        console.log('[IRON GRIND] cinematic start triggered');
        ...
        try {
            ...
            console.error('[IRON GRIND] timeline error:', err);
            // Fallback so user is never stuck
            document.body.innerHTML = '<div style="background:#05070a;color:#90e0ef;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;font-size:20px;text-align:center;padding:20px;">Timeline error — check console.<br><br>' + (err && err.message ? err.message : err) + '</div>';
        }
    }

    // Multiple input paths for mobile reliability
    startBtn.addEventListener('click', beginCinematic);
    startBtn.addEventListener('touchend', beginCinematic, { passive: false });
    startBtn.addEventListener('pointerup', beginCinematic);
    // Also allow tapping the whole start container
    startContainer.addEventListener('click', beginCinematic);
    startContainer.addEventListener('touchend', beginCinematic, { passive: false });
    ...
    console.log('[IRON GRIND] listeners attached — button is live');
}
